...
type GetAccessTokenFunc func(wc *Wechat) (accessToken string, err error)

//SetAccessTokenLock 设置读写锁（一个appID一个读写锁）
func (ctx *Wechat) SetAccessTokenLock(l *sync.RWMutex) {
	ctx.accessTokenLock = l
}

//SetGetAccessTokenFunc 设置自定义获取accessToken的方式, 需要自己实现缓存
func (ctx *Wechat) SetGetAccessTokenFunc(f GetAccessTokenFunc) {
	ctx.accessTokenFunc = f
}

//GetAccessToken 获取access_token
func (ctx *Wechat) GetAccessToken() (accessToken string, err error) {
	...
	accessTokenCacheKey := fmt.Sprintf("wechat.access_token.%s", ctx.AppID)
...
func (ctx *Wechat) GetAccessTokenFromServer() (resAccessToken ResAccessToken, err error) {
