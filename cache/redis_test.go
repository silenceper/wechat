...
// TestRedisBackwardCompatibility 测试向后兼容性
func TestRedisBackwardCompatibility(t *testing.T) {
	...
	t.Run("只设置MaxIdle应该映射到MinIdleConns", func(t *testing.T) {
		opts := &RedisOpts{
			Host:     server.Addr(),
			Database: 0,
			MaxIdle:  10,
	...
	t.Run("只设置MaxActive应该映射到PoolSize", func(t *testing.T) {
		opts := &RedisOpts{
			Host:      server.Addr(),
			Database:  0,
			MaxActive: 20,
	...
	t.Run("新字段应该优先于旧字段", func(t *testing.T) {
		opts := &RedisOpts{
			Host:         server.Addr(),
			Database:     0,
			MaxIdle:      5,
...
// TestRedisTimeoutConfiguration 测试超时配置
func TestRedisTimeoutConfiguration(t *testing.T) {
	...
	t.Run("正值超时应该正确应用", func(t *testing.T) {
		opts := &RedisOpts{
			Host:         server.Addr(),
			Database:     0,
			DialTimeout:  10,
	...
	t.Run("-1值应该禁用超时", func(t *testing.T) {
		opts := &RedisOpts{
			Host:         server.Addr(),
			Database:     0,
			DialTimeout:  -1,
	...
	t.Run("0值应该使用go-redis默认值", func(t *testing.T) {
		opts := &RedisOpts{
			Host:         server.Addr(),
			Database:     0,
			DialTimeout:  0,
