...
func (srv *OfficialServer) Render(bytes []byte) {
	// debug
...
func (srv *OfficialServer) String(str string) {
	writeContextType(srv.Writer, plainContentType)
	srv.Render([]byte(str))
}

// XML render to xml
func (srv *OfficialServer) XML(obj interface{}) {
...
func (srv *OfficialServer) Query(key string) string {
	value, _ := srv.GetQuery(key)
	return value
}

// GetQuery is like Query(), it returns the keyed url query value
func (srv *OfficialServer) GetQuery(key string) (string, bool) {
	req := srv.Request
